...
#READING IN DATA
################################################################################################################
data.set<-read.csv("C:/Users/abuga/Desktop/Metapopulation Manuscript Output/2000reps_PnLMnSimEq_3_50pnew2.csv")
################################################################################################################

data.set$sim.eq.size.r < data.set$avg.p.r
potato<-data.set[data.set$sim.eq.size.r < data.set$avg.p.r,]
head(potato)
potato<-subset(potato, initial.landscape.patch.ID>0)
head(potato)
potato<-potato[potato$time.to.eq.r!=1000,]
sum(potato$time.to.p1000.r < potato$time.to.eq.r)
head(potato[potato$time.to.p1000.r < potato$time.to.eq.r,])

data.set$sim.eq.size.e < data.set$avg.p.e
...
#this sometimes happens when p1000 is EXTREMELY small ~0.009 or smaller...why?
# AH! because when it takes over 950 generations to reach a very low eq
#most entries in those last 50 generations are 0 dragging p1000 as the average size across those 50 
#generations down lower, time to p1000 is calculated as +/- 1 patch so the bounds for time to p1000 
#are bigger than for avg.p.e, making it look like this value was hit earlier than it was
...
plot.Pi<-function(plot.data, landscape.type, title){
  ...
  print(ggplot(data=plot.data) + theme_classic() +
        ...
        #+ scale_fill_manual(values=rev(pal(8)), labels=c(rev(deltas$avg.disp), "Degradation"))) 
         + scale_fill_gradient("Dispersal Ability",low = pal(8),high = "light grey", trans="log", 
...
plot.P<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data) + theme_classic()
        ...
        + labs(x = "Percent Habitat Loss", y = "Avg. Expected Patch Occupancy at Equilibrium (P*)", 
               title = paste0(title, " Landscapes"))
        + scale_colour_gradient("Dispersal Ability",low = pal(8),high = "light grey", trans="log", 
                                breaks = c(100, 8, 4, 2, 1, 0.25, 0.125, 0.0625), 
                                labels = rev(c("1/16x", "1/8x", "1/4x", "Avg Min Nearest Neighbour", "2x", "4x", "8x", "Global Dispersal"))) #
        + scale_fill_manual(values=pal(8), guide="none")
        + theme(text = element_text(size=15))
...
dev.copy(png,'Dest Uniform Pstar.png')
dev.off()
plot2<-graph.P(data.set, "random", "Random")
dev.copy(png,'Dest Random Pstar.png')
dev.off()
plot3<-graph.P(data.set, "clustered", "More Clustered")
dev.copy(png,'Dest Clustered Pstar.png')
dev.off()
...
plot.sim<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data) + theme_classic()
        ...
        + labs(x = "Percent Habitat Loss", y = "Avg. Patch Occupancy after for last 50 of 1000 iterations (P1000)", 
               title = paste0(title, " Landscapes"))
...
dev.copy(png,'Dest Uniform P1000.png')
dev.off()
plot2<-graph.sim(data.set, "random", "Random")
dev.copy(png,'Dest Random P1000.png')
dev.off()
plot3<-graph.sim(data.set, "clustered", "More Clustered")
dev.copy(png,'Dest Clustered P1000.png')
dev.off()
...
plot.t<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data[complete.cases(plot.data[,1:7]),]) + theme_classic()
        + geom_ribbon(aes(x=percent.loss, ymin= log10(destruction.lower.CI), ymax=log10(destruction.upper.CI), group=(1/a), fill=factor(1/a)), alpha=0.3)
        + geom_line(aes(x = percent.loss, y = log10(destruction.median), group=(1/a), colour=1/(a)), size=1) 
        #+ geom_ribbon(aes(x=percent.loss, ymin= degradation.lower.CI, ymax=degradation.upper.CI, group=(1/a), fill=factor(1/a)), alpha=0.3)
        #+ geom_line(aes(x = percent.loss, y = degradation.median, group=(1/a), colour=1/(a)), size=1, linetype="dashed") 
        + labs(x = "Percent Habitat Loss", y = "Log Time to Expected Equilibrium", 
               title = paste0(title, " Landscapes"))
        + scale_colour_gradient("Dispersal Ability",low = pal(8),high = "light grey", trans="log", breaks = c(100, 8, 4, 2, 1, 0.5, 0.25, 0.125), labels = rev(c("1/8x", "1/4x", "1/2x", "Avg Min Nearest Neighbour", "2x", "4x", "8x", "Global Dispersal"))) #
        + scale_fill_manual(values=pal(8), guide="none")
        + theme(text = element_text(size=15))
...
dev.copy(png,'Dest Uniform log time to pstar.png')
dev.off()
plot2<-graph.t(data.set, "random", "Random")
dev.copy(png,'Dest Random log time to pstar.png')
dev.off()
plot3<-graph.t(data.set, "clustered", "More Clustered")
dev.copy(png,'Dest Clustered log time to pstar.png')
dev.off()
...
plot.tex<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data[complete.cases(plot.data[,1:7]),]) + theme_classic()
        ...
        #+ geom_ribbon(aes(x=percent.loss, ymin= degradation.lower.CI, ymax=degradation.upper.CI, group=(1/a), fill=factor(1/a)), alpha=0.3)
        #+ geom_line(aes(x = percent.loss, y = degradation.median, group=(1/a), colour=1/(a)), size=1, linetype="dashed") 
        + labs(x = "Percent Habitat Loss", y = "Log Time Extinct", 
               title = paste0(title, " Landscapes"))
        + scale_colour_gradient("Dispersal Ability",low = pal(8),high = "light grey", trans="log", breaks = c(100, 8, 4, 2, 1, 0.5, 0.25, 0.125), labels = rev(c("1/8x", "1/4x", "1/2x", "Avg Min Nearest Neighbour", "2x", "4x", "8x", "Global Dispersal"))) #
        + scale_fill_manual(values=pal(8), guide="none")
        + theme(text = element_text(size=15))
...
dev.copy(png,'Dest Uniform log time extinct.png')
dev.off()
plot2<-graph.tex(data.set, "random", "Random")
dev.copy(png,'Dest Random log time extinct.png')
dev.off()
plot3<-graph.tex(data.set, "clustered", "More Clustered")
dev.copy(png,'Dest Clustered log time extinct.png')
dev.off()
...
plot1<-graph.P(data.set, "regular", "More Uniform")
dev.copy(png,'Deg Uniform Pstar.png')
dev.off()
plot2<-graph.P(data.set, "random", "Random")
dev.copy(png,'Deg Random Pstar.png')
dev.off()
plot3<-graph.P(data.set, "clustered", "More Clustered")
dev.copy(png,'Deg Clustered Pstar.png')
dev.off()
...
plot1<-graph.sim(data.set, "regular", "More Uniform")
dev.copy(png,'Deg Uniform P1000.png')
dev.off()
plot2<-graph.sim(data.set, "random", "Random")
dev.copy(png,'Deg Random P1000.png')
dev.off()
plot3<-graph.sim(data.set, "clustered", "More Clustered")
dev.copy(png,'Deg Clustered P1000.png')
dev.off()

#time to expected eq GRAPH Deg ###############################################
tplot.data<-function(deltas, data){
  deltas<-na.omit(deltas)
  plot.data<-NULL
  for (i in 1:length(deltas$a)){
...
plot.t<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data[complete.cases(plot.data[,1:7]),]) + theme_classic()
        #+ geom_ribbon(aes(x=percent.loss, ymin= destruction.lower.CI, ymax=destruction.upper.CI, group=(1/a), fill=factor(1/a)), alpha=0.3)
        #+ geom_line(aes(x = percent.loss, y = destruction.median, group=(1/a), colour=1/(a)), size=1) 
        + geom_ribbon(aes(x=percent.loss, ymin= log10(degradation.lower.CI), ymax=log10(degradation.upper.CI), group=(1/a), fill=factor(1/a)), alpha=0.3)
        + geom_line(aes(x = percent.loss, y = log10(degradation.median), group=(1/a), colour=1/(a)), size=1) 
        + labs(x = "Percent Habitat Loss", y = "Log Time to Expected Equilibrium", 
               title = paste0(title, " Landscapes"))
        + scale_colour_gradient("Dispersal Ability",low = pal(8),high = "light grey", trans="log", breaks = c(100, 8, 4, 2, 1, 0.5, 0.25, 0.125), labels = rev(c("1/8x", "1/4x", "1/2x", "Avg Min Nearest Neighbour", "2x", "4x", "8x", "Global Dispersal"))) #
        + scale_fill_manual(values=pal(8), guide="none")
...
plot1<-graph.t(data.set, "regular", "More Uniform")
dev.copy(png,'Deg Uniform log time to pstar.png')
dev.off()
plot2<-graph.t(data.set, "random", "Random")
dev.copy(png,'Deg Random log time to pstar.png')
dev.off()
plot3<-graph.t(data.set, "clustered", "More Clustered")
dev.copy(png,'Deg Clustered log time to pstar.png')
dev.off()
##################################################################################################
...
plot1<-graph.tex(data.set, "regular", "More Uniform")
dev.copy(png,'Deg Uniform log time extinct.png')
dev.off()
plot2<-graph.tex(data.set, "random", "Random")
dev.copy(png,'Deg Random log time extinct.png')
dev.off()
plot3<-graph.tex(data.set, "clustered", "More Clustered")
dev.copy(png,'Deg Clustered log time extinct.png')
dev.off()
...
######## NOT WORKING YET. Don't really need
#time to expected extinction GRAPH
tplot.data<-function(deltas, data){
  data<-data[data$sim.eq.size.r==0 & data$avg.p.r==0,] #subset the data to only include entries where extinction occured for destruction and extinction was expected
  deltas<-na.omit(deltas)
  plot.data<-NULL
  for (i in 1:length(deltas$a)){
    df3<-plotdata.median.t.eq(data[data$alpha==deltas$a[i], ])
...
### NUMBER OF EXTINCTIONS ##########NOT WORKING, will take some time to fix but not necessary
explot.data<-function(deltas, data){
deltas<-na.omit(deltas)
  plot.data<-NULL
  for (i in 1:length(deltas$a)){
    df3<-plotdata.totalext(data[data$alpha==deltas$a[i], ])
    df3<-data.frame(a=rep(deltas$a[i], nrow(df3)), avg.disp=rep(deltas$avg.disp[i], nrow(df3)), df3)
    plot.data<-rbind(plot.data, df3)
  }
  return(plot.data)
}
#CHECK plot.data<-explot.data(deltas, data)

plot.ex<-function(plot.data, landscape.type, a, title){
  print(ggplot(data=plot.data[complete.cases(plot.data[,1:7]),]) + theme_classic()
        #+ geom_ribbon(aes(x=percent.loss, ymin= destruction.lower.CI, ymax=destruction.upper.CI, group=(1/a), fill=factor(1/a)), alpha=0.3)
        + geom_line(aes(x = percent.loss, y = total.extinctions, group=(1/a), colour=1/(a)), size=1) 
        #+ geom_ribbon(aes(x=percent.loss, ymin= degradation.lower.CI, ymax=degradation.upper.CI, group=(1/a), fill=factor(1/a)), alpha=0.3)
        #+ geom_line(aes(x = percent.loss, y = degradation.median, group=(1/a), colour=1/(a)), size=1, linetype="dashed") 
...
landscape.type="clustered"
title="More Clustered"
graph.ex<-function(data.set, landscape.type, title){
  data<-subset.landscape(data.set, landscape.type)
  a<-get.a(data)
  data<-clean.data(a, data)
  deltas<-get.deltas(a, data, landscape.type)
  plot.data<-explot.data(deltas, data)
  plot.tex(plot.data, landscape.type, a, title)
}

plot1<-graph.ex(data.set, "regular", "More Uniform")
dev.copy(png,'Dest Uniform # extinct.png')
dev.off()
plot2<-graph.ex(data.set, "random", "Random")
dev.copy(png,'Dest Random # extinct.png')
dev.off()
plot3<-graph.ex(data.set, "clustered", "More Clustered")
dev.copy(png,'Dest Clustered # extinct.png')
dev.off()
