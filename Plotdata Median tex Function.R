...
##############################################################################################################
plotdata.median.tex<-function(input.data){
  ...
  for (i in 1:length(bins)) {
    ...
    CI<-boot.ci(bootobject, conf=0.95, type="basic")}
  plot.data<-data.frame(percent.loss=bins, destruction.median, destruction.upper.CI, destruction.lower.CI, degradation.median, degradation.upper.CI, degradation.lower.CI)
  return(plot.data)
}
