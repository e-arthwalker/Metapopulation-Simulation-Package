...
vary.alpha.in.parallel<-function(alphas, landscape.type, n.landscapes, k, n.patches, landscape.limit, no.runs, scaled.lm){
  for (i in 1:length(alphas)) {
    ...
    output$rep.no<-k*(n.landscapes-1)+(k-(n.landscapes-1))+(output$rep.no-1)
