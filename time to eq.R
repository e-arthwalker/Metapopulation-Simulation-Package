...
#########################################################################################
SRLM.sim<-function(landscape, a, delta, timesteps, p.initial, p.star){
  ...
  if (any((eq.size-1) < metapop & metapop < (eq.size+1))) {
    time.to.eq<-which.max((eq.size-1) < metapop & metapop < (eq.size+1)) #finds first metapop entry with 0 patches occupied
  } else { time.to.eq<-NA }#time.extinct=NA if never hits 0
  data<-list(#metapop=metapop, #include metapop to check
    eq.size=eq.size, time.to.eq=time.to.eq)
  return(data) 
} 
#END SRLM FUNCTION
############################################################################################################################################################################
